...
CRCO 2/18/2021 - need to check IKN and IERRN after merge
      INTEGER, PARAMETER :: IFMAX=150, IKN=116, ISTRG=512, ILEN_FLD=200,
     &                      IERRN=341
...
C     character, such 'D' for Draft version.
      DATA VERSN /'21112'/   ! April 22, 2021
...
C***********************************************************************
     ...
     &     MFUNIT/19/, INCUNT/20/,  ! IBGUNT is now an array for BGSECTOR
     &     MPUNIT/21/, ISUNIT/22/, IPUNIT/23/, DBGUNT/24/, DBMUNT/25/,
     &     AREADBUNT/26/, PRMDBUNT/27/, ISUMUNT/28/, GDEPDBG/29/,
     &     PDEPDBG/30/, AWMADWDBUNT/31/,                                ! IO3UNT is now an array for O3SECTOR
...
C***********************************************************************
     ...
     & ERRMSG(  4)/'Keyword is Not Valid for This Pathway.  Keyword is'/
      DATA ERRCOD(  5)/'111'/,
     & ERRMSG(  5)/'User-specified minimum Sigma-V on LOW_WIND Keyword'/
      DATA ERRCOD(  6)/'112'/,
     & ERRMSG(  6)/'User-specified minimum WindSpeed on LOW_WIND Keywd'/
      DATA ERRCOD(  7)/'113'/,
     & ERRMSG(  7)/'User-specified maximum FRAN on the LOW_WIND Keywrd'/
      DATA ERRCOD(  8)/'114'/,
     & ERRMSG(  8)/'User-specified SZCOEF value on LOW_WIND Keyword   '/
      DATA ERRCOD(  9)/'115'/,
     & ERRMSG(  9)/'STARTING or FINISHED Out of Sequence:  Pathway =  '/
      DATA ERRCOD( 10)/'116'/,
     & ERRMSG( 10)/'Vector Wind Speeds specified on MODELOPT Keyword  '/
      DATA ERRCOD( 11)/'119'/,
     & ERRMSG( 11)/'Missing FINISHED-Runstream File Incomplete: ISTAT='/
      DATA ERRCOD( 12)/'120'/,
     & ERRMSG( 12)/'Pathway is Out of Sequence:  Pathway =            '/
      DATA ERRCOD( 13)/'130'/,
     & ERRMSG( 13)/'Missing Mandatory Keyword.  The Missing Keyword is'/
      DATA ERRCOD( 14)/'133'/,
     & ERRMSG( 14)/'LOW_WIND Option requires ALPHA option on MODELOPT '/
      DATA ERRCOD( 15)/'135'/,
     & ERRMSG( 15)/'Nonrepeatable Keyword or Recursed INCLUDED: Keywrd'/
      DATA ERRCOD( 16)/'136'/,
     & ERRMSG( 16)/'LOW_WIND ALPHA option selected on MODELOPT Keyword'/
      DATA ERRCOD( 17)/'137'/,
     & ERRMSG( 17)/'BETA option not allowed with DFAULT on MODELOPT '/
      DATA ERRCOD( 18)/'138'/,
     & ERRMSG( 18)/'ALPHA option not allowed with DFAULT on MODELOPT '/
C NOTE: Instead of renumbering a bunch of these, ERRCOD(19) and ERRMSG(19) 
C       are used below of #251
      DATA ERRCOD( 21)/'140'/,
     ...
     & ERRMSG( 46)/'Inconsistent temporally-varying BACKGRND options: '/
      DATA ERRCOD( 47)/'166'/,
     & ERRMSG( 47)/'SECTOR option invalid w/o BG/O3/NOx Inputs:       '/
      DATA ERRCOD( 48)/'167'/,
     ...
     & ERRMSG( 59)/'THRESH_1MIN 1-min ASOS wind speed threshold used  '/
      DATA ERRCOD( 60)/'187'/,
     & ERRMSG( 60)/'ADJ_U* Option for Stable Low Winds used in AERMET '/
      DATA ERRCOD( 61)/'188'/,
     & ERRMSG( 61)/'Non-DFAULT FLAT required for RLINE/RLINEXT source '/
      DATA ERRCOD( 62)/'189'/,
     ...
     & ERRMSG( 70)/'METHOD_2 for particulates is a non-DFAULT option  '/
      DATA ERRCOD( 71)/'198'/,
     & ERRMSG( 71)/'Non-DFAULT ALPHA Option Required for use of       '/
      DATA ERRCOD( 72)/'199'/,
     & ERRMSG( 72)/'Non-DFAULT BETA Option Required for use of        '/
      DATA ERRCOD( 73)/'200'/,
     ...
     & ERRMSG( 94)/'Missing Dist or Direction Setting In Polar Network'/
      DATA ERRCOD( 95)/'222'/,
     & ERRMSG( 95)/'SECTOR Value is out of order:                     '/
      DATA ERRCOD( 96)/'223'/,
...
C     the SRCGROUP, OLMGROUP, or PSDGROUP keywords
     ...
     & ERRMSG( 99)/'SrcID specified on PSDGROUP keyword not defined:  '/
      DATA ERRCOD(100)/'227'/,
     & ERRMSG(100)/'SECTOR Width is out of range:                     '/
      DATA ERRCOD(101)/'228'/,
...
C NOTE: Instead of renumbering a bunch of these, ERRCOD(19) and ERRMSG(19) 
C       are used, will need renumbering eventually!
      DATA ERRCOD( 19)/'251'/,
     & ERRMSG( 19)/'Deposition (DEPOS, DDEP, WDEP) incompatible with  '/
      DATA ERRCOD(124)/'252'/,
     ...
     & ERRMSG(133)/'Invalid shape defined (area=0) for AREAPOLY source'/
      DATA ERRCOD(134)/'267'/,
     & ERRMSG(134)/'RLINE/RLINEXT requires Zs = 0.0 or FLAT; SRCID=   '/
      DATA ERRCOD(135)/'271'/,
...
CCRT      DATA ERRCOD(164)/'305'/,
CCRT     & ERRMSG(164)/'Stack height > or = EPA formula height for SRCID: '/
     ...
     & ERRMSG(180)/'Particle Diameter Out-of-Range for Source         '/
      DATA ERRCOD(181)/'336'/,
     & ERRMSG(181)/'NO2RATIO Missing/Invalid for OLM/PVMRM/GRSM. Src:'/
      DATA ERRCOD(182)/'338'/,
     & ERRMSG(182)/'Neg Emis Rate Invalid with OLM/PVMRM/GRSM. Src: '/
      DATA ERRCOD(183)/'340'/,
...
C     buoyant line sources (Multiple_BuoyLines_D41_Wood)
      DATA ERRCOD(288)/'385'/,
     & ERRMSG(288)/'Following SRCID Included in Multiple BLPGROUPs:   '/
      DATA ERRCOD(289)/'388'/,
     & ERRMSG(289)/'Input buoyant line sources not in correct order:  '/
      DATA ERRCOD(290)/'389'/,
     & ERRMSG(290)/'Rotated buoyant line sources not in correct order:'/

      DATA ERRCOD(291)/'401'/,
     & ERRMSG(291)/'Use of turbulence data with ADJ_U* is NonDFAULT   '/

C --- New messages for v16216
      DATA ERRCOD(292)/'402'/,
     & ERRMSG(292)/'Turbulence data being used with ADJ_U* w/o DFAULT '/

      DATA ERRCOD(293)/'403'/,
     & ERRMSG(293)/'Turbulence data is being used w/o ADJ_U* option   '/

C --- More new messages for buoyant line processing; updated for multiple
C     buoyant line sources (Multiple_BuoyLines_D41_Wood)
      DATA ERRCOD(294)/'383'/,
     & ERRMSG(294)/'# buoy. lines in group not equal to # on HOUREMIS:'/
      DATA ERRCOD(295)/'476'/,
     & ERRMSG(295)/'# receptors within BL source (BLPGROUP)           '/
      DATA ERRCOD(296)/'477'/,
     & ERRMSG(296)/'Receptor inside BL source group for event:        '/

C     If at least one BL line is declared as urban, then all must be urban
      DATA ERRCOD(297)/'393'/,
     & ERRMSG(297)/'Not all lines in BL source declared urban in group'/

C --- New messages for AWMADWNW and ORD_DWNW keywords on CO pathway
      DATA ERRCOD(298)/'121'/,
     & ERRMSG(298)/'Duplicate option specified for Keyword            '/
      DATA ERRCOD(299)/'122'/,
     & ERRMSG(299)/'AWMADWNW Option requires ALPHA option on MODELOPT '/
      DATA ERRCOD(300)/'123'/,
     & ERRMSG(300)/'ORD_DWNW Option requires ALPHA option on MODELOPT '/
      DATA ERRCOD(301)/'124'/,
     & ERRMSG(301)/'UEFF conflict: AWMADWNW and ORD_DWNW keywords'/
...
C      DATA ERRCOD(302)/'125'/,
      DATA ERRCOD(302)/'126'/,
     & ERRMSG(302)/'STREAMLINE requires AWMAUTURB or AWMAUTURBHX'/

      DATA ERRCOD(303)/'471'/,
     & ERRMSG(303)/'Met. ref hgt WS < 1.0 m/s; set to 1.0 for BL srcs '/

      DATA ERRCOD(304)/'472'/,
     & ERRMSG(304)/'Release hgt < 2.0m for BL source; set to 2.0m for '/
     
    
C Multiple_BuoyLines_D41_Wood
C     Messages to accomodate multiple buoyant line processing.
      DATA ERRCOD(305)/'254'/,
     & ERRMSG(305)/'SrcID specified on BLPGROUP keyword not defined:  '/

      DATA ERRCOD(306)/'255'/,
     & ERRMSG(306)/'Non-BL source specified in a BLPGROUP'/

      DATA ERRCOD(307)/'257'/,
     & ERRMSG(307)/'BL SourceID in more than one BLPGROUP'/

      DATA ERRCOD(308)/'258'/,
     & ERRMSG(308)/'BL SourceID not in a BLPGROUP'/

      DATA ERRCOD(309)/'502'/,
     & ERRMSG(309)/'BLPGROUP ID not found among BLPINPUT group IDs'/

      DATA ERRCOD(310)/'503'/,
     & ERRMSG(310)/'BLPGROUP IDs do not match BLPINPUT IDs'/

      DATA ERRCOD(311)/'504'/,
     & ERRMSG(311)/'BLPINPUT group ID already specified-must be unique'/

      DATA ERRCOD(312)/'505'/,
     & ERRMSG(312)/'BLPINPUT records out of order, BLPINPUT Groups:'/


C D_32 Wood - Check for non-parallel lines within group
      DATA ERRCOD(313)/'506'/,
     & ERRMSG(313)/'BUOYLINE not parallel to 1st line of its group:'/
...
C     MESSAGES FROM D32.  CHANGE FROM 305 TO 314
      DATA ERRCOD(314)/'125'/,
     & ERRMSG(314)/'ONE OR MORE PATHS HAS FINISHED KEYWORD MISSING'/

      
C     JAT D068 1/15/21:  UDPATED DEPOSITION MESSAGE AFTER NEWER MESSAGES
      DATA ERRCOD(315)/'473'/,
     & ERRMSG(315)/'Default deposition parameter(s) used for SRCID:   '/


C     JAT 1/29/21  D070 TURBULENCE OPTIONS
C     ADD WARNING MESSAGE AND INFORMATIONAL MESSAGES
      DATA ERRCOD(335)/'443'/,
     & ERRMSG(335)/'SELECTED TURBULENCE OPTION:                       '/
      
      DATA ERRCOD(336)/'444'/,
     & ERRMSG(336)/'TURBOPT INVALID WITH DFAULT; RESET                '/
      
      DATA ERRCOD(337)/'445'/,
     & ERRMSG(337)/'Set sigma-theta to missing for                    '/
      DATA ERRCOD(338)/'446'/,
     & ERRMSG(338)/'Set sigma-w to missing for                        '/
...
CCRT avoid conflict with merging code 
      DATA ERRCOD(316)/'127'/,
     & ERRMSG(316)/'User-specified minimum Sigma-W on LOW_WIND Keyword'/

CRCO 9/28/2020, D061 User BIGT
CCRT 02/16/2021: D061 User BIGT - Update array index for to
CCRT avoid conflict with merging code 
      DATA ERRCOD(317)/'129'/,
     & ERRMSG(317)/'User-specified BIGT on LOW_WIND Keyword'/

CCRT  2/22/2021: D059 AWMA Downwash Options
CCRT  Resolve conflict with error array index and code      
      DATA ERRCOD(318)/'478'/,
     & ERRMSG(318)/'AWMAUTurb & AWMAUTurbHX entered; AWMAUTurbHX used '/


CRCO 2/25/2021 need to resolve error code numbers     
C---- CERC 11/30/20 New messages for GRSM option
      DATA ERRCOD(319)/'600'/,     
     & ERRMSG(319)/'Keyword Invalid w/o PVMRM/OLM/GRSM/TTRM:        '/
      DATA ERRCOD(320)/'601'/,
     & ERRMSG(320)/'Error calculating travel time for GRSM option:  '/
      DATA ERRCOD(321)/'602'/,
     & ERRMSG(321)/'Following Keyword Invalid Without GRSM:         '/
      DATA ERRCOD(322)/'603'/,
     & ERRMSG(322)/'Not Enough NOx Concentrations Specified           '/
      DATA ERRCOD(323)/'604'/,
     & ERRMSG(323)/'NOXVALUE, NOX_VALS or NOX_FILE Keyword Needed for '/
      DATA ERRCOD(324)/'605'/,
     & ERRMSG(324)/'Both NOXVALUE and NOX_VALS keywords are specified '/
      DATA ERRCOD(325)/'606'/,
     & ERRMSG(325)/'Inconsistent temporally-varying NOX_VALS options: '/
      DATA ERRCOD(326)/'607'/,
     & ERRMSG(326)/'No Hrly NOx & No Sub; Assume zero conc,   KURDAT ='/
      DATA ERRCOD(327)/'608'/,
     & ERRMSG(327)/'Date/time Mismatch: NOX_FILE File, KURDAT/Sector ='/
      DATA ERRCOD(328)/'609'/,
     & ERRMSG(328)/'No Hrly NOx & No Sub; Use zero conc,      KURDAT ='/
      DATA ERRCOD(329)/'610'/,
     & ERRMSG(329)/'NOXSUB for missing hourly NOX val, KURDAT/Sector ='/
      DATA ERRCOD(330)/'611'/,
     & ERRMSG(330)/'NOXFIL w/o NOXVALs; zero bgd for hrs wth miss NOx '/
      DATA ERRCOD(331)/'612'/,
     & ERRMSG(331)/'No NOx bgd.  NOx bgd calc from NO2 assuming equil '/
      DATA ERRCOD(332)/'613'/,
     & ERRMSG(332)/'NO2 bgd value > NOx bgd value. Setting NOx = NO2. '/
      DATA ERRCOD(333)/'614'/,
     & ERRMSG(333)/'Src type not recognised in instant. plume calc.   '/
      DATA ERRCOD(334)/'615'/,
     & ERRMSG(334)/'Error allocating variables for plume size calc.   '/

CCRT  3/15/2021: Moved from above and updated array indices  
CCRT  (304 and 305 were already in use above)
      DATA ERRCOD(339)/'710'/,
     & ERRMSG(339)/'TTRM NO2 processing not currently configured for '/
      DATA ERRCOD(340)/'720'/,
     & ERRMSG(340)/'Non-fatal Error Occurs Writing to the File of  '/


      DATA ERRCOD(341)/'620'/,
     & ERRMSG(341)/'Both RLINE barriers on same side of SRCID: '/
...
C     Meteorological variables 
      ...
      DOUBLE PRECISION  :: WSPD_ADJ, SIGZ_Y
...
C     Run option parameters
      DOUBLE PRECISION, PARAMETER  :: ERROR_LIMIT = 1.0E-3
      DOUBLE PRECISION, PARAMETER  :: FAC_DISPHT  = 5.0D0 
...
C     XSHIFT      = amount to shift x if in the recirculation zone
