...
! GAMFACT is saved to TTRMOUT(IREC,ISRC,17) 70 lines below 
...
C     Initialize TTRMVAL(NUMTYP) array, if allocated
      TTRMINST(:) = 0.0D0
      TTRMFRAC(:) = 0.0D0
      TTRMNO2(:)  = 0.0D0
      TTRMSUB(:)  = 0.0D0
...
C              Calculate in-stack converted NO2 and available NOx 
               NO2 = ANO2_RATIO(ISRC) * CHI(IREC,ISRC,1)
               NOX = (1.0D0-ANO2_RATIO(ISRC))*CHI(IREC,ISRC,1)
...
!                Check on whether the TTRM fraction is equal to or less than the NO2Equil (default set to 90%)
                       IF (TTRMFRAC(ITYP) .GE. NO2Equil) THEN                    
                         ...
                         HRVAL(ITYP)   = NO2Equil*CHI(IREC,ISRC,1)
                         NO2           = 0.0
                         NOX           = CHI(IREC,ISRC,1)
...
C              Calculate in-stack converted NO2 and available NOx 
               NO2 = ANO2_RATIO(ISRC) * CHI(1,ISRC,1)
               NOX = (1.0D0-ANO2_RATIO(ISRC))*CHI(1,ISRC,1) 
!
!     Perform TTRM for POINT, VOLUME or AREA type sources
               IF ((SRCTYP(ISRC)(1:5) .EQ. 'POINT') .OR.
     &             (SRCTYP(ISRC) .EQ. 'VOLUME') .OR.
...
! If requested, write output to debug file
     ...
     &                TTRMOUT(1,ISRC,8),  TTRMOUT(1,ISRC,9), TTRMSRC,
...
C
C        PURPOSE: Steps through Hourly Calcs for the Ozone Reaction Rate 
C                 and saves intermediary variables for an optional debug
C                 file.
C
C        PROGRAMMER: Carlos Szembek.
C
