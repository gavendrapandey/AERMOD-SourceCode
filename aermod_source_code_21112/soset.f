...
C        Initialize Counters and Set Status Switch
         ...
         NUMBLGRPS = 0         ! # BL groups defined by BLPGROUP records - D41_Wood
...
C ---       This is the first BLPGROUP record encountered
            CONT = .FALSE.
         ELSE
C ---       This is not the first BLPGROUP defined; check on whether
C           this record is a continuation for the same BLPGROUP
            DO I = 1, NUMBLGRPS
...
C ---       BLPGROUP ALL has been specified
C ---       Check for whether all sources have been defined yet,
C           based on NSRC determined during PRESET vs. NUMSRC
C           determined during normal SETUP.
C           IF (NUMSRC .LT. NSRC) THEN
C              Issue fatal error message; BLPGROUP ALL out of order
C              CALL ERRHDL(PATH,MODNAM,'E','140','OLMGROUP ALL')
C              GO TO 999
C           ELSE
