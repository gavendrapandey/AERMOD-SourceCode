...
C              No NO2STACK card specified for PVMRM, OLM or GRSM options.
...
C          CERC 11/30/20
CRCO 2/25/2021 Check for ALPHA and DFAULT conflicts
            GRSM = .TRUE.
            IF (.NOT. L_ALPHA) THEN
C              WRITE Error Message     ! ALPHA Option Required for GRSM
               CALL ERRHDL(PATH,MODNAM,'E','198',KOPT)
            END IF 
            IF (DFAULT) THEN
C              WRITE Error Message     ! Non-DEFAULT Option Conflict
...
CCRT  1/29/2018: Added L_ALPHA to conditions for non-default options
     ...
     &     FASTALL .OR. LOW_WIND .OR. L_ALPHA .OR. GRSM
