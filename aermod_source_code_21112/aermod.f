...
C=======================================================================
C
C       This revised version of AERMOD (dated 21112) includes numerous
...
!     Added for TTRM, AECOM - Feb. 2021
      IF (TTRMDBG) THEN
        IF( .NOT.EVONLY )THEN
          WRITE ( TTRMUNT, 6010 )           
 6010     FORMAT (' KURDAT,XREC,YREC,SRCGRP,SRCID,O3CONC (μg/m3),',
     &'O3CONC (ppb),Amb. Temp. (K),k1, time coefficent,Distance (m),',
     &'Effective WSPD (m/s),Transit Time (s),Plume Type,',
     &'Downwind Distance,Radial Distance,Crosswind Distance,UEFF,',
     &'UEFFD,UEFF3,FRAN,PPF,GAMFACT,CHI-Indirect,CHI-DIRECT,',
     &'TOTAL NOx (as NO2),Instack-NO2,Available NO (as NO2),',
     &'TTRM FRACTIONAL,TTRM Converted NO2,Source Sub-total NO2')
        ELSE IF(EVONLY)THEN
          WRITE ( TTRMUNT, 6011 )
...
!      ALLOCATE (TTRMSRC(NSRC,8), STAT=IASTAT) ! TTRMSRC(SRCID,distance_ueff combination based on stability & plume type)
      ALLOCATE (TTRMINST(NTYP), TTRMFRAC(NTYP), TTRMSUB(NTYP), 
     &          TTRMNO2(NTYP), L_TTRMSRCTYP(NSRC),STAT=IASTAT)
...
C        Process Cards to Determine Storage Requirements
     ...
     &                FIELD(I) .EQ. 'WDEP') THEN
                     NTYP = NTYP + 1
                  END IF
C                 Set PVMRM/OLM/ARM2/GRSM logicals for use in ALLSETUP
