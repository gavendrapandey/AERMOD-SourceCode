...
C*****************************************************************
        IF (URBAN .AND. POLLUT .NE. 'NO2') THEN
     ...
     *               ' Used for NO2 Conversion'
                 WRITE(IOUNT,9890) NO2Equil
9890                 FORMAT('            with an Equilibrium ',
     &               'NO2/NOx Ratio of ',F6.3)
                 WRITE(IOUNT,9891) NO2Stack
9891                 FORMAT('            and with a Default',
     &               ' In-stack Ratio of ',F6.3)
                 IF (PSDCREDIT) THEN
                     WRITE(IOUNT,9099)
                     WRITE(IOUNT,*)'           ALPHA Option for ',
     &                       'Calculating PSD Increment'
                     WRITE(IOUNT,*)'           Consumption with',
     &                       ' PSD Credits Selected.'
                 END IF
             ELSE IF (OLM) THEN
                 WRITE(IOUNT,*) '        6. Ozone Limiting Method',
     &               ' (OLM) Used for NO2 Conversion'
                 WRITE(IOUNT,98901) NO2Equil
98901                FORMAT('            with an Equilibrium ',
     &               'NO2/NOx Ratio of ',F6.3,' and')
                 IF (NUMOLM .GT. 0) THEN
                     WRITE(IOUNT,98911) NUMOLM
98911                FORMAT('            with ',I3,' OLMGROUP(s)')
                 ELSE
                     WRITE(IOUNT,98912)
98912                FORMAT('            with NO OLMGROUPs')
                 END IF
             ELSE IF (RUNTTRM) THEN
                 WRITE(IOUNT,*) '          6. Ozone Reaction Rate',
     &               ' (TTRM) Used for NO2 Conversion'
             ELSE IF (ARM2) THEN
                 WRITE(IOUNT,*) '        6. Ambient Ratio ',
     &               'Method Ver 2 (ARM2) Used for NO2 Conversion'
                 WRITE(IOUNT,9980) ARM2_Min
9980             FORMAT('               with a Minimum NO2/NOx ',
     &              'Ratio of ',F6.3)
                 WRITE(IOUNT,9981) ARM2_Max
9981             FORMAT('                and a Maximum NO2/NOx ',
     &              'Ratio of ',F6.3)
             ELSE
...
90912             FORMAT('            with NO OLMGROUPs')
               END IF
            ELSE IF (RUNTTRM) THEN
               WRITE(IOUNT,*) '          6. Trav Time Reaction Method ',
     &                 'Used for NO2 Conversion'
               WRITE(IOUNT,90901) NO2Equil
            ELSE IF (ARM2) THEN
               WRITE(IOUNT,*) '        6. Ambient Ratio ',
...
9991           FORMAT('                and a Maximum NO2/NOx ',
     &                 'Ratio of ',F6.3)
            ELSE IF (GRSM) THEN
C              CERC 11/30/20
               WRITE(IOUNT,*) '        6. GRS Chemistry ',
     &                'Method (GRSM) Used for NO2 Conversion'
            ELSE
                    WRITE(IOUNT,*) '        6. Full Conversion ',
     &                 'Assumed for NO2.'
            END IF
         END IF

         IF (.NOT.PVMRM .AND. .NOT.OLM .AND. .NOT. RUNTTRM .AND.        
     &       .NOT.ARM2 .AND. .NOT.GRSM .AND. POLLUT.NE.'NO2') THEN
            IF (URBAN) THEN
...
C     OPTIONS SUMMARY
C     JAT 1/29/21 D070 ADD TURBOPTS TO LIST
     ...
     &                                   L_CCVR_Sub .OR. L_TEMP_Sub .OR.
     &                                  L_RDEPRESS .OR. L_RBARRIER .OR.
     &    TURBOPTS(1) .OR. TURBOPTS(2) .OR. TURBOPTS(3) .OR. TURBOPTS(4)
     &    .OR. TURBOPTS(5) .OR. TURBOPTS(6) .OR. TURBOPTS(7) .OR. 
     &    TURBOPTS(8) .OR. TURBOPTS(9)) THEN
         WRITE(IOUNT,9099)
     ...
     &                   'which forces calculation of centerline values'
      ...
      IF (LOW_WIND) THEN
        WRITE(IOUNT,*)'        LOW_WIND - Use LOW_WIND ALPHA option'
        IF (L_UserSVmin .OR. L_UserWSmin .OR. L_UserFRANmax .OR.
     &      L_UserSWmin .OR. L_UserBigT) THEN
           WRITE(IOUNT,*)
