...
C           hour based on the wind direction)
         ...
         IF (NBLINES. GE. 1) THEN
            CALL BLRECP                                   !  ---   CALL BLRECP          
         END IF
...
C                 receptors when a buoyant line source is processed
C
...
C           Rotate
            EY = -EX*SINTCOR + EY*COSTCOR
            YR_SCS(I) = EY
            EX = (EX + EY*SINTCOR)/COSTCOR
            XR_SCS(I) = EX
         END DO
...
C         calculations by setting a flag for each receptor
         YLMAX = YS_SCS(1)
         YLMIN = YS_SCS(NBLINES)
         XLMAX = BLINEPARMS(1)%XEND_TR1
         XLMIN = BLINEPARMS(1)%XBEG_TR1
         DO NL = 1,NBLINES
            XLMIN = MIN(XLMIN,BLINEPARMS(NL)%XBEG_TR1)
            XLMAX = MAX(XLMAX,BLINEPARMS(NL)%XEND_TR1)
            YLMIN = MIN(YLMIN,YS_SCS(NL))
            YLMAX = MAX(YLMAX,YS_SCS(NL))
         END DO

C        Define the vertices of the bounding rectangle that defines the 
C        exclusion zone - coordinates are specified counterclockwise
         NVRECT = 4
         BLREC_X(1) = XLMIN
         BLREC_X(2) = XLMAX
         BLREC_X(3) = XLMAX
...
C         is inside the exclusion zone
         DO NR = 1,NUMREC
            IF( YR_SCS(NR) .LE. (YLMAX + 0.1D0) .AND. 
     &          YR_SCS(NR) .GE. (YLMIN - 0.1D0) .AND.
     &          XR_SCS(NR) .LE. (XLMAX + 0.1D0) .AND. 
     &          XR_SCS(NR) .GE. (XLMIN - 0.1D0)) THEN
               NRECIN = NRECIN + 1
               BL_RFLAG(NR) = .true.
            END IF
         END DO
         WRITE(DUMMY,'("NREC=",I5)') NRECIN
