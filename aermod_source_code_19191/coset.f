...
C ---    Check for pollutant ID = 'NO2' for PVMRM, OLM, and ARM2 options
         IF ((PVMRM .OR. OLM .OR. ARM2) .AND.
     &                                          POLLUT .NE. 'NO2') THEN
...
C RCO 7/24/2019
C CTEMP Remove this code in next version of model (after 19191)
C Commenting out warning message for LOW_WIND and FRANMAX option. 
C FRANMAX is an optional keyword, no need to isssue warning. 
C ---       Issue warning message, unless user has specifed a value
C           for FRANMAX on the LOW_WIND keyword
C            IF (.NOT. L_UserFRANmax) THEN
C               CALL ERRHDL(PATH,MODNAM,'W','133','0.9D0    ')
C            END IF
...
CCRT  1/29/2018: Added L_ALPHA to conditions for non-default options
     ...
     &     FASTALL .OR. LOW_WIND .OR. L_ALPHA .OR. .NOT.
     &     L_UrbanTransition)) THEN
...
C     note that PVMRM, OLM, ARM, ARM2, and DEPOS debug files are opened
C     elsewhere
...
CCRT  AWMAUTURB flag is FALSE.
      IF (L_STRMLN_BLDG .AND. .NOT. L_AWMA_UTurb) THEN
...
C      the height at which to compute the efffective wind speed.
...
C --- ORD_DWNW and AWMADWNW and have different ways to define
C      the height at which to compute the efffective wind speed.
C      One or the other can be specified, or neither, but BOTH CANNOT
C      be specified.  Check for that here.  A similar check is performed
C      in ORD_DOWNWASH

      IF (L_ORD_UEFF .AND. L_AWMA_UEFF) THEN
C ---    Write error message
         CALL ERRHDL(PATH,MODNAM,'E','124','  ')
      END IF
