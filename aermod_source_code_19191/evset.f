...
C        Translate event receptor
         XR_SCS(1) = AXR(IEV) - XOR
         YR_SCS(1) = AYR(IEV) - YOR
         EX = XR_SCS(1)
         EY = YR_SCS(1)
...
C         better placement of the code would eleimante this redundancy
         YLMAX = YS_SCS(1)
...
C         source; write a message if the receptor is inside the rectanlge;
C         this should not be the case if the event was generated by 
C         an AERMOD run
         IF( YR_SCS(1) .LE. YLMAX .AND. YR_SCS(1) .GE. YLMIN .AND.
     &       XR_SCS(1) .LE. XLMAX .AND. XR_SCS(1) .GE. XLMIN) THEN
             BL_RFLAG(1) = .true.
            WRITE(DUMMY,'(A10)')
