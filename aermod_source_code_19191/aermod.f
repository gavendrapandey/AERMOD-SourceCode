...
C                         Draft Version Dated v19191
C 
C                               July 10, 2019
C
C               *** SEE AERMOD MODEL CHANGE BULLETIN MCB#14 ***
C
...
C=======================================================================
C
C       This revised version of AERMOD (dated 19191) includes several
C       modifications relative to the previous version (dated 18081); 
C       see AERMOD Model Change Bullentin (MCB) #14 and AERMOD User's 
C       Guide. 
C
C https://www.epa.gov/scram/air-quality-dispersion-modeling-preferred-and-recommended-models#aermod
C
C-----  MODIFIED BY:    Chris Owen
C                       U.S. EPA, OAQPS/AQAD
C                       Air Quality Modeling Group
C
C                       July 10, 2019
C
C-----  MODIFIED FROM:  AERMOD (Version Dated 18081)

C
C-----  Bug Fixes:
C
C       1.  BACKGROUND Concentrations output units Modified subroutine 
C           SUMBACK to convert background concentrations to the model 
C           output units requested via the EMISUNIT or CONCUNIT 
C           keywords. Background concentrations are converted 
C           internally in AERMOD to μg/m3 but were not being converted 
C           to the requested output units when model outputs were not 
C           μg/m3.  Background concentrations are converted to the 
C           requested model output units by dividing EMIFAC(1) BY 
C           1.0E6 (the default conversion of grams to mg) in SUMBACK.
C           
...
C           Washout ratio is calculated based on the fine mass 
C           fraction, and an assumed diameter of 6 microns for the 
C           coarse mode.  The washout ratio is used to calculate the 
C           collision efficiency which is then used to calculate the 
C           scavenging ratio.  Previously, AERMOD used particle 
C           density and settling velocity for the fine particle to 
C           calculate wet deposition parameters when using Method 1 or 
C           2 for particle deposition.  These were inconsistent with 
C           the Method 2 inputs in that density is not an input for 
C           Method 2 and Method 2 dry deposition assumes a settling 
C           velocity of 0 m/s for the fine particle mode.
C       
C       4.  Buoyant Line Source Minimum Release Height and Minimum 
C           Wind Speed
C           A minimum release height of 2.0 meters is imposed.  A 
C           buoyant line source release height of 0.0 meters caused 
C           AERMOD to run very slowly and produce 0.0 ug/m3 for all 
C           hours and receptors.  A minimum reference height wind 
C           speed of 1.0 m/s is imposed.  This value is consistent 
C           with the meteorology that was produced with the CRSTER 
C           meteorological processor, the met processor for the 
C           Buoyant Line and Point (BLP) model.
C
C       5.  MODELOPT Number 6
C           Previously, the AERMOD.out summary file lists two No. 6 
C           user-specified options when user selects non-DFAULT and 
C           URBANOPT. One of the two messages incorrectly indicates a 
C           NO2 conversion is applied when non-NO2 POLLUTID used.  An 
C           if statement was added for NO2 processing in inpsum.f to 
C           remove the duplicated message.
C
C       6.  DFAULT with ADJ_Ustar
C           Previously, a W402 warning message was returned in the 
C           AERMOD.out summary file when the DFAULT MODELOPT was used 
C           without use of the AERMET ADJ_U*.  The block of code 
C           responsible for the incorrect warning message was 
C           eliminated in PFLCNV subroutine of metext.f.
C
C       7.  ERRMSG(18)
C           ERRMSG(18) was defined three times in modules.f, using 
C           ERRHDL numbers 133, 137, and 138, which resulted in some 
C           compilers (g95) not functioning properly.  The modules.f 
C           file was reorganized sequentially and ERRMSG(18) usage of 
C           ERRHDL 137 and 138 was removed.  ERRHDL 137 and 138 were 
C           replaced with ERRMSG 204 in the coset.f.
C
C       8.  LOW_WIND Undefined ERROR Message
C           Previously, an undefined ERROR message and “ELWD” error 
C           code was returned in the AERMOD.out summary file when the 
C           LOW_WIND MODELOPT was used without the ALPHA MODELOPT. 
C           The ERROR message code was modified to “E133” in coset.f.
C
C       9.  ELEVUNIT
C           Previously, the optional ELEVUNIT keyword to convert 
C           elevation units from feet to meters was not applied to LINE 
C           or BUOYLINE sources.  The SOLOCA subroutine in soset.f was 
C           modified to apply the feet to meters conversion used for 
C           other source types to LINE and BUOYLINE sources.
C
C-----  Enhancements:
C

C       1.  ARM2 Enabled with BETA RLINE and ALPHA RLINEXT Source Types
C           ARM2 was extended for application to sources and source 
C           groups that include RLINE and RLINEXT line sources.
C
C       2.  EVENT Processing Enabled with BETA RLINE and ALPHA RLINEXT 
C           Source Types
C           EVENT processing was extended for application to sources 
C           and source groups that include RLINE and RLINEXT line 
C           sources.
C
C       3.  Urban Stability Enabled with BETA RLINE and ALPHA RLINEXT 
C           Source Types
...
C           Previously, AERMOD treated a buoyant line source in an 
C           urban environment as a source in a rural environment, as 
C           is done in the Buoyant Line and Point (BLP) model.  The 
C           capability to process a buoyant line source in an urban 
C           environment was added.  The surface roughness (SFCZ0), 
C           Monin-Obukhov length (OBULEN), and mixing height (ZI) are 
C           adjusted for an urban environment and a new value of the 
C           Pasquill-Gifford (P-G) stability category (KST) is 
C           computed using the subroutine LTOPG.
C
C-----  Formulation updates - Regulatory:
C       
C           None
C
C-----  Formulation updates - BETA:
C       
...
C           requires the BETA and FLAT model options. The RLINE source 
C           type has identical inputs to the LINE source type
C
C           R-LINE model: current version 1.2, last updated November 
C           2013 (https://www.cmascenter.org/r-line).
C
C-----  Formulation updates - ALPHA:
C       
C
C       1.  Enhanced Building Downwash Options
C           Options to examine the effects of enhanced building 
C           downwash algorithms for point sources was added.  The user 
C           can selectively apply one or more of these options in the 
C           AERMOD input control file.  Two new keywords are added to 
C           the CO pathway: ORD_DWNW and AWMADWNW.  Each of these 
...
C           calculated for main plume concentrations; ORDTURB to 
C           control an adjustment for the non-dimensional vertical 
C           turbulence intensity, wiz0, from 0.6 to 0.7; and ORDCAV to 
C           shift the point at which the vertical and lateral 
C           dispersion coefficients begin to grow with downwind 
C           distance from the lee edge of the building to the end of 
C           the cavity.  Parameters available for the AWMADWNW keyword 
C           are: AWMAUEFF to control the height at which an effective 
C           wind speed is calculated for main plume concentrations; 
C           AWMAUTURB to specify new lower and upper bounds for 
C           calculating the effective parameters ueff, sweff, sveff, 
C           and tgeff; and STREAMLINE (or STREAMLINED) to perform 
C           downwash for a streamlined building such as a cooling 
C           tower. Any combination of parameters is allowed EXCEPT 
C           ORDUEFF and AWMAUEFF cannot both be specified in the same 
C           model run. In addition, STREAMLINE (or STREAMLINED) 
C           requires the AWMAUTURB option to also be specified. If any 
C           of these keywords and parameters are used, the ALPHA option 
C           must be specified on the MODELOPT record.  An optional 
C           debug file is available for the options associated with the 
C           AWMADWNW keyword by specifying AWMADW on the DEBUGOPT 
C           keyword. If none of these options are applied, the standard 
C           AERMOD building downwash algorithms will be used.
C
C       2.  RLINEXT Source Type
C           The RLINEXT source type was added to model roadways, or 
C           similar line-type releases, using the dispersion 
C           calculations from the R-LINE model (version 1.2) and 
C           requires the ALPHA and FLAT model options. The RLINEXT 
C           source has inputs and capabilities identical to the 
C           R-LINE model. The RLINEXT source has capability to model 
C           depressed roadways and roadside barriers; the RLINE 
C           source does not have these capabilities.
C
C           R-LINE model: current version 1.2, 
C           last updated November 2013 
C           (https://www.cmascenter.org/r-line).
C
C       3.  Method 2 particle deposition and gas deposition ALPHA 
C           Option 
...
C           with the DFAULT option as it is based on a method from a 
C           previous model, the Acid Deposition and Oxidant model (ADOM)


C=======================================================================
C
C----- NOTE: Summaries of AERMOD model changes prior to version 19191 
C            have been moved to aermod_mcbs.txt file. Also, an archive 
C            of prior AERMOD MCB's is maintained on the EPA SCRAM 
C            website.
C
C=======================================================================
C
...
C     and ARM2, and write file header information. First
...
C           contributions for max daily 1-hour averages
     ...
     &                         'Rec Arrays for MAXDCONT!'
            ...
            IF (L_BLSOURCE) THEN
              ALLOCATE (XR_SCS_SAV(NREC), YR_SCS_SAV(NREC), 
     &                  BL_RFLAG_SAV(NREC), STAT=IBSTAT)
            ENDIF
            IF (IBSTAT .NE. 0) THEN
               CALL ERRHDL(PATH,MODNAM,'E','409','Setup Arrays')
               ALLOC_ERR = .TRUE.
...
C        Initialize MOVES input unit flag and FIRSTHR flag 
     ...
     &             XR_RCS(NREC), YR_RCS(NREC), BL_RFLAG(NREC), 
     &             STAT=IASTAT)
         IF (IASTAT .NE. 0) THEN
            CALL ERRHDL(PATH,MODNAM,'E','409','Setup Arrays')
            ALLOC_ERR = .TRUE.
...
C     Variable Initializations
      ...
      NURB    = 0
      NVMAX   = 0
      NBLINES = 0          ! Number of lines in a buoyant line source
      ILINE   = 0
...
C ---    Add variable for number of RLINE sources
         NRLINES = 0
C ---    Add variable for number of lines in buoyant line source
         NBLINES = 0
...
C ---          only the source IDs for AREACIRC sources.
         ...
         ELSE
C ---       Invalid SrcTyp
            CALL ERRHDL(PATH,MODNAM,'E','CCC',FIELD(3))
         END IF
