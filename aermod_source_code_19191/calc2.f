...
C --- Calculate time scale (s) for random dispersion, based on 24 hours
         TRAN = BIGT * 3600.0D0
...
C     Calculate FSUBZ from V;  FSUBZ = V / (SQRT(2*PI) * SZ)
      ...
      IF (AWMADWDBG) THEN
         WRITE(AWMADWDBUNT,*) 'VRTSBN SZ:     ',SZARG ,
     &                     ', ZR: ',ZR,', FSUBZ: ',FSUBZ
      END IF
...
C                 OLM, and PVMRM options for modeling NO2
C
C        PROGRAMMER: Roger Brode
C
C        DATE:    May 14, 2014
...
C
C        CALLED FROM:   ARM2_CALC, OLM_CALC and PVMRM_CALC
C
C***********************************************************************
...
C ---       Include background for this source group
            BACKAVE(IGRP) = BACKAVE(IGRP) + BGCONC
            IF (PERIOD .OR. ANNUAL) THEN
               BACKANN(IGRP) = BACKANN(IGRP) + BGCONC
            END IF
            IF (ISEAHR(IGRP) .EQ. 1) THEN
               BACKSEASHR(IGRP,ISEAS,IHOUR) =
     &         BACKSEASHR(IGRP,ISEAS,IHOUR) + BGCONC
            END IF

C           Begin Averaging Period LOOP
            DO IAVE = 1, NUMAVE
               AVEVAL(IREC,IGRP,IAVE,ITYP) = BGCONC +
     &                                   AVEVAL(IREC,IGRP,IAVE,ITYP)
            END DO
C           End Averaging Period LOOP
            IF (PERIOD .OR. ANNUAL) THEN
               ANNVAL(IREC,IGRP,ITYP) = BGCONC +
     &                                      ANNVAL(IREC,IGRP,ITYP)
            END IF
            IF (ISEAHR(IGRP) .EQ. 1) THEN
               SHVALS(IREC,IGRP,ISEAS,IHOUR,ITYP) = BGCONC +
     &         SHVALS(IREC,IGRP,ISEAS,IHOUR,ITYP)
            END IF
         END IF
      END DO
...
CCRFL     &       /,9X,6(1X,G12.4),/,9X,3(1X,G12.4))
