...
c --- Apply reduction in wake wind speed
      ...
      ufac=1.0D0
      dufac=0.0D0
      if(ipositn.LT.4) call WAKE_U(.FALSE.,xb,yb,zb,ufac,dufac,DBGUNT)
      ua=ufac*ua0
...
c --- Extract met and apply reduction in wake wind speed
      ...
      ufac=1.0D0
      dufac=0.0D0
      if(ipositn.LT.4) call WAKE_U(ldbu,xb,yb,zb,ufac,dufac,DBGUNT)
      ua=ufac*ua0
      dudz=ufac*dudz0+dufac*ua0
...
c ---       Normal debug output
            if(ldbhr) then
               write(DBGUNT,*) ' '
               write(DBGUNT,*)'NUMRISE call to WAKE_DFSN'
...
C     Debugging for AWMADWNW
      IF (AWMADWDBG) THEN
         write (AWMADWDBUNT, '(A, 3(A, F7.3))') 'wake_turb:',
     &           '  sweff=', sweff, '  ueff=', ueff, '  ambiz=', ambiz
      END IF
...
c-----------------------------------------------------------------------
     ...
     &   (scope, x, znew, ambiz, ambiy, du_ua, tiz, tiy)
                                       !-----  ---  ---
c-----------------------------------------------------------------------
...
c --- CALLS:      None
c-----------------------------------------------------------------------
      USE MAIN1, ONLY: L_STRMLN_BLDG, L_RECT_BLDG, 
     &                 AWMADWDBUNT, AWMADWDBG
      USE prime_wakedat, ONLY: HB, WB, HR, xLb
...
c    ! args
      ...
      DOUBLE PRECISION, INTENT (IN)    :: x,znew, ambiz, ambiy
...
c-----Get the value of z. If argument znew is >= 0, that is it.
...
c---- Compute du_ua as needed by wake_u and wake_turb
      ...
      if (scope == 'vel') then
         if (AWMADWDBG) write (AWMADWDBUNT, '(3A, 7(A, f7.3))')
     &                  'scope=', scope, ':',
     &                  '  xml=', xml,
     &                  '  znew(inarg)=', znew,
     &                  '  z=', z,
...
c      Otherwise, continue to compute tiz, tiy as needed by wake_turb.
      ...
      tiz = ambiz * MIN (50.d0, A1)
      tiz = MIN (0.9d0, tiz)
      ! Eq (1b)
      A1 = (1.d0 + SV) / (1.d0 - du_ua)
      tiy = ambiy * MIN (50.d0, A1)
      tiy = MIN (0.9d0, tiy)

      IF( AWMADWDBG ) write (AWMADWDBUNT, '(3A, 19(A, f7.3))')
     &   '  scope=', scope, ':',
     &   '  x=', x,
     &   '  znew(inarg)=' ,znew,
     &   '  z=', z,
