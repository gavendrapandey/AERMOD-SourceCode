...
C***********************************************************************
      ...
      INTEGER, PARAMETER :: IFMAX=150, IKN=101, ISTRG=512, ILEN_FLD=200,
     &                      IERRN=304
C*#
...
C***********************************************************************
     ...
     &   'MAXDCONT','NOHEADER','AWMADWNW','ORD_DWNW'/
...
C               32 = armratio                      43 = blavgval
C               33 = awmadwnw                      50 = finished
C               34 = ord_dwnw
C               50 = finished
C
...
C --- New messages for buoyant line processing
      DATA ERRCOD(288)/'385'/,
     & ERRMSG(288)/'BLPGROUP keywrd not operational; all BLP lines are'/
      DATA ERRCOD(289)/'388'/,
...
C --- More new messages for buoyant line processing
      DATA ERRCOD(294)/'383'/,
     & ERRMSG(294)/'# buoyant lines does not match # in hourly emiss: '/
      DATA ERRCOD(295)/'476'/,
...
C     If at least one BL line is declared as urban, then all must be urban
      DATA ERRCOD(297)/'393'/,
     & ERRMSG(297)/'At least one BL line is urban - ALL must be urban '/
...
CCRT  6/14/2019: AWMADWNW STREAMLINE option is only applicable with UTurb option
      DATA ERRCOD(302)/'125'/,
     & ERRMSG(302)/'AWMADWNW STREAMLINE requires AWMAUTURB option'/
...
C     DCLWALL     = barrier distance from center line
...
C     Run option parameters
      ...
      DOUBLE PRECISION, PARAMETER  :: FAC_DISPHT  = 5.0 
...
C     Source variables
      ...
      DOUBLE PRECISION  :: XSBEGIN, YSBEGIN, ZSBEGIN ! begining points of line source
      DOUBLE PRECISION  :: XSEND, YSEND, ZSEND       ! end points of line source
C     INDQ        = source index
C     SIGMAY0     = initial horizontal dispersion coefficient
C     UEFF        = initial vertical dispersion coefficient
C     HRELEASE    = source height
C     XSBEGIN     = x-coordinate of beginning point of line
C     YSBEGIN     = y-coordinate of beginning point of line
C     XSBEGIN     = z-coordinate of beginning point of line
C     XSEND       = x-coordinate of end point of line
C     YSEND       = y-coordinate of end point of line
C     ZSEND       = z-coordinate of end point of line

C     Computation parameters
      DOUBLE PRECISION, PARAMETER  :: PI     = 3.14159265358979
      DOUBLE PRECISION, PARAMETER  :: SM_NUM = 1.0E-8
      DOUBLE PRECISION, PARAMETER  :: XD_MIN = 1.0D0  
C     PI          = value of PI used for RLINE source type computations
...
C
      LOGICAL              :: L_BLSOURCE,L_BLURBAN
      LOGICAL, ALLOCATABLE :: BL_RFLAG(:)
      INTEGER              :: NBLINES, NUMBL, BL_NUMURB

C     NBLINES   = number of lines in the buoyant line source
C     NUMBL     = number of buoyant line sources  (currenly limited to 1)
C     BL_RFLAG  = flag indicating if a receptor is inside (true) or out-
C                 side (false) the extents of the buoyant line source
...
C       receptor is in the exclusion zone of the buoyant line source
C       when MAXDCONT is used
