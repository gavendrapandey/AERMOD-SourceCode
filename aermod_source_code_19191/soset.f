...
C --- Check for BLPGROUP keyword, which is not operational yet
...
C            are processed prior to making any computations and 
...
C     Variable Declarations
      ...
      LOGICAL :: FOPEN
      INTEGER :: I, J, N, ITOTSRC, ITOTGRP, NumBack, LNUM
      INTEGER :: ILSAVE, NERRCOUNT, HRLYBLCOUNT
...
C           For now AERMOD only processes one buoyant line source
            NUMBL = 1
         END IF
...
C           Apply a minimum release height of 2.0 meters to the buoyant line
...
C     Check that the number of lines defined in the control file match
C      the number of lines in the hourly emission file.  Write an error 
C      message if there is a mismatch (this requires that the hourly
C      emissions file always have emissions for all buoyant lines0
      IF (HRLYBLCOUNT .GT. 0) THEN
         IF (NBLINES .GT. 0) THEN
            IF (HRLYBLCOUNT .NE. NBLINES) THEN
               CALL ERRHDL(PATH,MODNAM,'E','383','BUOYLINE')
            END IF
         END IF
      END IF 

C     If there is a buoyant line source (existence based on NBLINES),
C      be sure the endpoints are specified in the correct order, west 
C      to east, for the BLP algorithms.
...
C        If one line of a buoyant line source is declared urban, then all
C         lines must be declared urban.  If not, generate an error message.
C         L_BLURBAN is initialized as FALSE
         DO ILINE = 1,NBLINES
            IF (URBSRC(BLINEPARMS(ILINE)%ISRCNUM) .EQ. "Y") THEN
               IF (.NOT. L_BLURBAN) L_BLURBAN = .TRUE.
               BL_NUMURB = BL_NUMURB + 1
            END IF
         END DO
         
         IF ((L_BLURBAN) .AND. (BL_NUMURB .NE. NBLINES)) THEN
C           Write Error Message:  At least one but not all BL lines declared as urban
            CALL ERRHDL(PATH,MODNAM,'E','393','BUOYLINE')
         END IF
...
C ---       Check on number of BUOYLINE sources
...
C     Variable Initializations
      ...
      IF (SRCTYP(ISDX) .EQ. 'RLINE') THEN
        ...
        RLSOURCE(ISDX)%HTWALL = 0.0D0         ! barrier height
        RLSOURCE(ISDX)%DCLWALL = 0.0D0        ! barrier distance from center line
...
C      to E-05 (approx. equal to 0.5 degrees)
      if (DABS(COSTCOR) .lt. 1.0D-10) COSTCOR = 1.0D-05
      DO IL = 1,NBLINES
...
C     ERROR handling for barrier parameters
      IF (RLSOURCE(INDEXS)%HTWALL.LT. 0.0D0) THEN
