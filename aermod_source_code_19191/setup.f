...
C        Extract Keyword From Field 2                       ---   CALL EXKEY
         ...
         IF (NOKEY .AND. FIELD(2) .EQ. 'BLPGROUP') THEN
            KEYWRD = PKEYWD
...
C --- Initialize logicals for background ozone and concentrations
      L_O3File(:)   = .FALSE.
      L_O3Hourly    = .FALSE.
      L_O3VAL(:)    = .FALSE.
      L_O3VALUES(:) = .FALSE.
...
C --- Initialize O3VARY arrays across all sectors to -99.0 to facilitate QA checks on
C     completeness of the inputs
