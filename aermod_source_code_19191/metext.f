...
C           so user doesn't get misleading warning message
     ...
     &          INDEX(BUFFER,'MIFF-Mod') .GT. 0 )THEN
...
C              Compute gridded profile of epsilon for PVMRM option
               IF (PVMRM) THEN
                  GRIDEPS = -99.0D0
               END IF
...
C ---    Hour is calm or missing; set IO3SECT = 0
         IO3SECT = 0
      ELSE
...
C ---       Compare to minimum value PARAMETER, SVMIN = 0.2
...
C     population of 2 million. 
      DOUBLE PRECISION, PARAMETER :: REFZI = 400.0D0
...
C        Compute Urban Heat Flux, and recalculate Monin-Obukhov length
         URBHF  = 0.03D0 * DELTUR * USTAR
         RHO    = 101325.D0/(287.04D0*TA)

C        Compute Urban WSTAR
         URBWSTR(IURB) = ((G/TA) * URBHF * ZIURB(IURB)) ** THIRD
...
C        rural and urban surface roughness length.
         ...
         URBUSTR(IURB) = MAX( USTAR, URBWSTR(IURB) *
...
C        Compute equivalent Monin-Obukhov length
         URBOBULEN(IURB) = -((TA*URBUSTR(IURB)**3)/(0.4D0*G*URBHF))
