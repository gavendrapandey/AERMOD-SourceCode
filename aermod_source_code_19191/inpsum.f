...
9991           FORMAT('                and a Maximum NO2/NOx ',
     ...
     &       .NOT.ARM2 .AND. POLLUT.NE.'NO2') THEN
            IF (URBAN) THEN
...
C     Write Out Numbers of Sources, Groups, and Receptors for This Run
 ...
 9063       FORMAT(10X, 'Streamlined Buildings')
      ...
      IF (L_ORDDW) THEN
         WRITE (IOUNT, 9065)
 9065    FORMAT(1X,'**ORD Downwash Options Specified:')
         IF (L_ORD_Ueff) THEN
            WRITE (IOUNT, 9066)
 9066       FORMAT(10X, 'ORDUEFF')
         END IF
         IF (L_ORD_Turb) THEN
            WRITE (IOUNT, 9067)
 9067       FORMAT(10X, 'ORDTURB')
         END IF 

         IF (L_ORD_Cav) THEN
            WRITE (IOUNT, 9068)
 9068       FORMAT(10X, 'ORDCAV')
         END IF
      END IF 
...
C --- Summarize DEBUGOPTs selected by the user
     ...
     &    .or. OLMDEBUG .or. ARM2DEBUG .or. DEPOSDBG )THEN
...
89077 FORMAT(1X,A12,2X,A5,2X,E11.5,4F10.1,A8,F9.2,1X,F9.2,
 ...
 9112 FORMAT(//14X, 'BARRIER',5X,'BARRIER',
     & /3X,'SOURCE',5X,'HTWALL',6X,'DCLWALL',
     & /4X,'ID',7X,'(METERS)',5X,'(METERS)',
     & /66(' -')/)
     
 9113 FORMAT(1X,A12,1X,F6.2,8X,F6.2)
