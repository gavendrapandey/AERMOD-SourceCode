...
C           are skipped for a PVMRM, OLM, or ARM2 run; the
...
CRWB     R. Brode, PES, 8/15/96
         ...
         IF( L_AWMA_UTurb ) sw30 =  max (sw30, swmin)

         SVEFF = MAX( SVEFF, SVMIN, SVUMIN*UEFF )
         IF( L_AWMA_UTurb ) sv30 =  max (sv30, svmin, svumin*u30)

         IF( L_VECTORWS )THEN
            UEFF  = DSQRT( UEFF*UEFF + 2.0D0*SVEFF*SVEFF )
            IF( L_AWMA_UTurb ) u30 = dsqrt (u30*u30 +2.d0*sv30*sv30)
         ENDIF

         UEFF  = MAX( UEFF, WSMIN )
         IF( L_AWMA_UTurb ) u30 = MAX (u30, wsmin)
...
C Unused:      INTEGER :: J, K, INOUT
      ...
      DOUBLE PRECISION   :: BL_UREF, BL_UEFF, BL_THETA
      ...
      INTEGER :: ISRC_SAV, KST_SAV, BL_URBKST
C     Variable Initializations
      MODNAM = 'BL_CALC'
      JITCT  = 0
      IWPBL  = 0
...
C       is not exited until all bouyant lines are processed
      ISRC_SAV = ISRC

C     Recall that LTOPG is called in METEXT.f in subr.SET_METDATA for a 
C       buoyant line source; save that value in case it is changed for
C       an urban source
      KST_SAV  = KST
...
C ---       Call LTOPG for a buoyant line source in an urban area
         ...
         ELSE
C           KST remains the rural/original value
            URBSTAB = .FALSE.
            ZI = ZIRUR
            OBULEN = RUROBULEN
         END IF
...
C     If there is an hourly emissions file for buoyant lines, calculate
C       the average FPRIME (buoyancy parameter) from the values in the file

      IF (QFLAG(BLINEPARMS(1)%ISRCNUM) .EQ. 'HOURLY') THEN
         SUMFPRM = 0.0D0
         BUOY_LINES1: DO LNUM = 1, NBLINES
            ISRC = BLINEPARMS(LNUM)%ISRCNUM
            SUMFPRM = SUMFPRM + AFP(ISRC)
         END DO BUOY_LINES1
         BLAVGFPRM = SUMFPRM / DBLE(NBLINES)
      END IF
...
C        Compute the profiled wind speed at the buoyant line release height
...
C           OF THE LINE)
            ...
            DIFF = DABS(SUM2-SUMM)
            IF (DIFF*CUQ .LT. 0.1D0) GO TO 720
            CORR = DIFF/SUM2
...
C           skipping calculations with PVMRM, OLM, or ARM2.
...
C     Restore the source number back to the original value from the saved value
C      value as well as the PG stability, mixing height, Obukhov length, and
C      roughness length 
      ISRC = ISRC_SAV
      KST  = KST_SAV
      ZI   = ZIRUR
...
C***********************************************************************
C             BL_DBTSIG Module of the AMS/EPA Regulatory Model - AERMOD
C
...
C***********************************************************************
C             BL_RISE Module of the AMS/EPA Regulatory Model - AERMOD
C
...
C***********************************************************************
C             BL_ZRISE Module of the AMS/EPA Regulatory Model - AERMOD
C
...
C       seven points (BL_XDIST(1-7)), as computed in BL_CAL just before
C       entering the loop over the buoynat lines.
C
C     INPUT:
C       X    Distance X
C
...
C***********************************************************************
C             BL_GAUSS Module of the AMS/EPA Regulatory Model - AERMOD
C
C
C     INPUT:
C       ISTAB    PG stability
...
C***********************************************************************
C             BL_WSC Module of the AMS/EPA Regulatory Model - AERMOD
C
...
C        CONSTANT 0.6 = BETA
         Z = (P2 * BLAVGBHGT**P * DBLE(NBLINES) * BLAVGFPRM /
     &       (0.6D0 * BLAVGBLEN * UM * S))**(1.0D0/P2)

C        CONSTANT 3.3333333 = 2./BETA WITH BETA=0.6
         U = 3.3333333D0 * DBLE(NBLINES) * BLAVGFPRM / (BLAVGBLEN*S*Z*Z)
         U = DMAX1(U,UM)
      ENDIF
...
C***********************************************************************
C             BL_LENG Module of the AMS/EPA Regulatory Model - AERMOD
C
...
C        CONSTANT 1.5915494 = 3./(PI*BETA) WITH BETA=0.6
     ...
     &              DXM833 * DXM833 * (DXM833 + 1.5915494D0*BLAVGLWID)
         ...
         IF (XI .GT. BLAVGBLEN) THEN
            XI = BLAVGBLEN/2.0D0 +
     &               DSQRT(12.0D0*T1 - 3.0D0*BLAVGBLEN*BLAVGBLEN)/6.0D0
